...
The second partial derivative test is a renowed technique for determining local extremum points. This mathematical method had been well studied since the beginning era of calculus, and has been used extensively both in the academic and in real-world applications. Yet, upon studying the ridge problem of hill climbing\cite{ridge}, we stumbled on a thought that the same issue could present in the forementioned test under a stochastic space setup.

In this work, we rigorously prove that such problem exists, and propose an analytical solution for it. We hope our finding pushes the knowledge boundary of stochastic gradient descent in specific, and optimization in general. We expect this to help researchers and practitioners surpass false local extremum points and navigate down true optimization objectives, consolidating the robustness of theories and systems.
