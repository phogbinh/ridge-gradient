...
\begin{definition}
    ...
    \end{align*}
    where $\vec{x} = \point{x_{1}, x_{2}, \dots, x_{n}}$. The directional function of $f$ at point $\vec{x_{0}}$ in direction $\vec{v} \in \Rn$ is the single variable function
    \begin{align*}
...
\end{equation}
where $\underset{n \times n}{\mat{A}} \succ 0$. For any $\vec{v} \in \Rn$, our directional function at $\vec{x_{0}} = \vec{0}$ is
\begin{align}
    \df{v}\point{k} & = f\point{\vec{x_{0}} + k\vec{v}}\\
